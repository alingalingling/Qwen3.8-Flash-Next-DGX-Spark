...
| UD-IQ4_XS(4-bit)| 93.7 GB | 91.1% | 0.079 | 112 GB | 临界(余 ~9 GiB)|
...
> 仅验证 8K 短窗口 × 2 流;262K 长窗口并发未验证(0xBakeer 的崩溃可能发生在长上下文路径),
> 生产大窗口并发前需先 128K 级验证。
...
- **cafe-llama.cpp fork 路线永久放弃**:两次加载 MTP 草稿均爆内存死机(90GB 与 82GB 主模型各一次),增量远超文档估算的 10-15GB

### 5.3 窗口/质量横向对照(2026-08-27 夜;最新全投机方案表见 5.1a)
...
| Q4_K_XL + PLE-offload 基线(仅8K窗口) | 22.9 | 20.1 | 20.0 | **82 GB** | 93.5% |
| Q4_K_XL + PLE + ngram-mod | 47.4 | 19.3 | 20.0 | **82 GB** | 93.5% |
| Q4_K_XL + PLE + MTP+ngram-mod(仅8K窗口) | **70.1** | 19.3 | 39.1 | 86 GB | **93.5%** |
...
> **窗口限制(事故 5)**:Q4+PLE-offload 只验证过 8K;262K 加载即整机冻结——Q4 仅限短窗口,生产 262K 只用 Q3 配方。
...
## 9. 结论速览(2026-08-28 终版)

1. ** 终版生产推荐:Q3_K_XL + NVMe-PLE + MTP+ngram-map-k4v**(262K 窗口,`--parallel 2` 已实测安全)
 - 代码复制 **78.9 t/s 单流 / 77.5 聚合(2 并发)** = 旧配置(24 t/s)的 **3.3 倍**;内存 **70GB**(原 102GB),余 51GB
 - 命令:见 deploy_playbook(方案 A);就绪后 warm_table.py 预热 PLE 表
2. ** 投机全解锁(2026-08-28)**:ngram-map-k4v 零成本代码复制 **84.2 t/s**(+237%,接受率 0.849);**MTP+map-k4v 叠加 108.4 t/s(+334%,基线 4.3 倍)**;散文 21.5-29.6 增益有限(收益=输出可预测性)
3. ** 并发可用(修正 0xBakeer 报告)**:parallel 2 聚合 93-104 t/s(8K)/ 77.5(262K),0 断言崩溃;4 流未测(内存允许)
4. ** Q4_K_XL 借助 NVMe-PLE 可跑**:82GB(官方需求 112GB),93.5% 质量;+MTP+map-k4v 代码 **70.1 t/s**; 仅 8K 短窗口(事故 5)
5. **262K 上下文免费**(架构红利),但大窗口拖慢短 prompt prefill(实现税+架构税)
